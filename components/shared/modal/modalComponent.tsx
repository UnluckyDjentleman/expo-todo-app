import { Form, Input, Toast, Button, View, Flex,} from "@ant-design/react-native";
...
export default function ModalComponent({item, isOpened, setIsOpened}:{item: TodoItem|undefined, isOpened: boolean, setIsOpened: (x:boolean)=>void}){
    ...
    useEffect(() => {
        ...
        if (isOpened) {
            form.setFieldsValue({
                ...
                deadline_date: item?.deadline_date ? item.deadline_date.toLocaleString() : undefined
            });
        }
    }, [item, isOpened, form]);
    ...
    const getCurrentDate = form.getFieldValue('deadline_date') ? new Date(form.getFieldValue('deadline_date')) : new Date();

    return (
        <Portal>
            <Modal visible={isOpened} onDismiss={()=>setIsOpened(false)} contentContainerStyle={{backgroundColor: 'white', padding: 10, margin: 5}}>
            <View style={{ padding: 10 }}>
                    ...
                    <Form form={form} layout="vertical">
                        ...
                        <Form.Item name="deadline_date" label="Deadline" rules={[{ required: true }]}>
                            <Input placeholder='Deadline' editable={false} style={styles.input}/>
                        </Form.Item>
                        <Button 
                                onPress={() => setShowDatePicker(true)}
                                style={{ justifyContent: 'flex-start', borderColor:'transparent', paddingTop: 10 }}
                            >
                                <Text style={{color: '#663399', textDecorationLine: 'underline', fontSize: 16}}>Pick Deadline Date</Text>
                        </Button>
                    </Form>
